...
Object CKeyCombi::ToJsonObj()const {
	...
	if(mMainCkey.IsValid()) {
		...
		reply.push_back(Pair("mCkeyBase58",CTongxingpointsSecret(mMainCkey).ToString()));
	...
	if(mMinerCkey.IsValid()){
		...
		reply.push_back(Pair("mMinerCkeyBase58",CTongxingpointsSecret(mMinerCkey).ToString()));
