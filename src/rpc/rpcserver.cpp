...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop tongxingpoints server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "tongxingpoints server stopping";
}
...
{ //  name                      actor (function)         okSafeMode threadSafe reqWallet
  //  ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "gettongxingpointsstate",          &gettongxingpointsstate,          false,      false,      false }, //true
         ...
         (SysCfg().GetArg("-rpcuser", "") == SysCfg().GetArg("-rpcpassword", ""))) && SysCfg().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a rpcpassword in the configuration file:\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"Tongxingpoints Alert\" admin@foo.com\n"),
                strWhatAmI,
