...
Value createcontracttx(const Array& params, bool fHelp) {
	...
	if (fee > 0 && fee < CTransaction::nMinTxFee) {
	   cout << "fee " << fee << " CTransaction::nMinTxFee "<< CTransaction::nMinTxFee << endl;
...
Value getaccountinfo(const Array& params, bool fHelp) {
	...
	Object obj;
	{
		...
		if (accView.GetAccount(userId, account)) {
            ...
            vector<unsigned char> vec6 = account.regID.GetVec6();
            obj.push_back(Pair("vec6", strprintf("%x %x %x %x %x %x",vec6[0], vec6[1],vec6[2], vec6[3], vec6[4], vec6[5])));
...
//						"1. \"address \"  (string, required) The Tongxingpoints address.\n"
//						"\nResult:\n"
//						"\"account info\"  (string) \n"
...
//		CTongxingpointsAddress address(params[0].get_str());
//		CKeyID keyid;
//		if (!address.GetKeyID(keyid))
//			throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Tongxingpoints address");
//
//		CUserID userId = keyid;
...
Value gencheckpoint(const Array& params, bool fHelp)
{
	...
	CTongxingpointsSecret vchSecret;
...
Value validateaddress(const Array& params, bool fHelp)
{
	if(fHelp || params.size() != 1)
		{
			throw runtime_error(
					 "isvalideaddess \"tongxingpoints address\"\n"
					 "\ncheck address is valide\n"
					 "\nArguments:\n"
					 "1. \"tongxingpoints address\"  (string, required) tongxingpoints address\n"
					 "\nResult:\n"
					 "\nExamples:\n"
