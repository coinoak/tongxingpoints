...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"Tongxingpointsaddress\"   (string, required) The Tongxingpoints address for the private key\n"
            "\nResult:\n"
    ...
    CTongxingpointsAddress address;
    ...
    if (!pwalletMain->GetKey(keyID, minerkey,true))
           throw JSONRPCError(RPC_WALLET_ERROR, "Private key for address " + strAddress + " is not known");
    Object reply;
    	reply.push_back(Pair("privkey", CTongxingpointsSecret(vchSecret).ToString()));
    if(minerkey.ToString() != vchSecret.ToString())
    	reply.push_back(Pair("minerkey", CTongxingpointsSecret(minerkey).ToString()));
    else
...
Value dumpwallet(const Array& params, bool fHelp) {
	...
	Object reply;
	reply.push_back(Pair("created by Tongxingpoints", CLIENT_BUILD + CLIENT_DATE));
