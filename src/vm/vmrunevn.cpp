...
//		assert(0);
	...
	if(tx->llFees <= CBaseTransaction::nMinTxFee) {
		LogPrint("VM", " tx->llFees is %d mintxFee is %d", tx->llFees, CBaseTransaction::nMinTxFee);
...
//		vector_unsigned_char accountid(it.accountid,it.accountid+sizeof(it.accountid));
		...
		if(accountid.size() == 6){
			...
			string tongxingpointsaddr(accountid.begin(), accountid.end());
			userkeyid = CKeyID(tongxingpointsaddr);
