...
static RET_DEFINE ExGetBase58AddrFunc(unsigned char * ipara,void * pVmEvn){
    ...
    if(!GetData(ipara,retdata) ||retdata.size() != 1 || retdata.at(0).get()->size() != 6)
    {
	 ...
	 string tongxingpointsaddr = addrKeyId.ToAddress();

	 auto tem =  make_shared<std::vector< vector<unsigned char> > >();
	vector<unsigned char> vTemp;
	vTemp.assign(tongxingpointsaddr.c_str(),tongxingpointsaddr.c_str()+tongxingpointsaddr.length());
...
enum CALL_API_FUN {
	...
	GET_TONGXINGPOINTS_ADDRESS_FUN,
};
...
const static struct __MapExterFun FunMap[] = { //
		...
		{GET_TONGXINGPOINTS_ADDRESS_FUN,ExGetBase58AddrFunc},
