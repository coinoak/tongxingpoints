...
int LogPrintStr(const char* category, const string &str) {
	...
	if (SysCfg().IsPrintToFile()) {
		...
		pathDebug = GetDataDir() / file;   // /home/share/bille/TONGXINGPOINTS_test/regtest/INFO.log
...
#else
	const char* pszModule = "Tongxingpoints";
#endif
	if (pex)
		return strprintf(
...
boost::filesystem::path GetDefaultDataDir() {
	...
	// Windows < Vista: C:\Documents and Settings\Username\Application Data\Tongxingpoints
	// Windows >= Vista: C:\Users\Username\AppData\Roaming\Tongxingpoints
	// Mac: ~/Library/Application Support/Tongxingpoints
	// Unix: ~/.Tongxingpoints
#ifdef WIN32
	// Windows
	return GetSpecialFolderPath(CSIDL_APPDATA) / "Tongxingpoints";
#else
	fs::path pathRet;
...
boost::filesystem::path GetConfigFile() {
	boost::filesystem::path pathConfigFile(CBaseParams::GetArg("-conf", "tongxingpoints.conf"));
			...
			if (!fDone) {
				...
				if (!fMatch) {
					...
					string strMessage = _("Warning: Please check that your computer's date and time "
							"are correct! If your clock is wrong Tongxingpoints will not work properly.");
