...
private:
	CTongxingpointsAddress *addr;
public:
	CDacrsAddressVisitor(CTongxingpointsAddress *addrIn) :
			addr(addrIn) {
	}
...
bool CTongxingpointsAddress::Set(const CKeyID &id) {
...
bool CTongxingpointsAddress::Set(const CTxDestination &dest) {
	return boost::apply_visitor(CDacrsAddressVisitor(this), dest);
}

bool CTongxingpointsAddress::IsValid() const {
...
CTxDestination CTongxingpointsAddress::Get() const {
	if (!IsValid())
...
bool CTongxingpointsAddress::GetKeyID(CKeyID &keyID) const {
	uint160 id;
...
//bool CTongxingpointsAddress::GetRegID(CRegID &Regid) const {
//
////	vector<unsigned char> vid;
////	vid.push_back(CBaseParams::ACC_ADDRESS);
...
bool CTongxingpointsAddress::IsScript() const {
	return IsValid() && vchVersion == SysCfg().Base58Prefix(CBaseParams::SCRIPT_ADDRESS);
}

void CTongxingpointsSecret::SetKey(const CKey& vchSecret) {
...
CKey CTongxingpointsSecret::GetKey() {
	CKey ret;
	ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
	return ret;
}

bool CTongxingpointsSecret::IsValid() const {
	bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
	bool fCorrectVersion = vchVersion == SysCfg().Base58Prefix(CBaseParams::SECRET_KEY);
	return fExpectedFormat && fCorrectVersion;
}

bool CTongxingpointsSecret::SetString(const char* pszSecret) {
	return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CTongxingpointsSecret::SetString(const string& strSecret) {
