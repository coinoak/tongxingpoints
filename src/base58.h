...
//
#ifndef TONGXINGPOINTS_BASE58_H
#define TONGXINGPOINTS_BASE58_H
...
/** base58-encoded Tongxingpoints addresses.
 * Public-key-hash-addresses have version 0 (or 111 testnet).
 * The data vector contains RIPEMD160(SHA256(pubkey)), where pubkey is the serialized public key.
 * Script-hash-addresses have version 5 (or 196 testnet).
 * The data vector contains RIPEMD160(SHA256(cscript)), where cscript is the serialized redemption script.
 */
class CTongxingpointsAddress : public CBase58Data {
public:
    bool Set(const CKeyID &id);
    bool Set(const CTxDestination &dest);
    bool IsValid() const;

    CTongxingpointsAddress() {}
    CTongxingpointsAddress(const CTxDestination &dest) { Set(dest); }
    CTongxingpointsAddress(const string& strAddress) { SetString(strAddress); }
    CTongxingpointsAddress(const char* pszAddress) { SetString(pszAddress); }
 ...
 */
class CTongxingpointsSecret : public CBase58Data
{
public:
    ...
    CTongxingpointsSecret(const CKey& vchSecret) { SetKey(vchSecret); }
    CTongxingpointsSecret() {}
};

template<typename K, int Size, CBaseParams::Base58Type Type> class CTongxingpointsExtKeyBase : public CBase58Data
{
public:
    ...
    CTongxingpointsExtKeyBase(const K &key) {
        SetKey(key);
    }

    CTongxingpointsExtKeyBase() {}
};

typedef CTongxingpointsExtKeyBase<CExtKey, 74, CBaseParams::EXT_SECRET_KEY> CTongxingpointsExtKey;
typedef CTongxingpointsExtKeyBase<CExtPubKey, 74, CBaseParams::EXT_PUBLIC_KEY> CTongxingpointsExtPubKey;

#endif // TONGXINGPOINTS_BASE58_H
