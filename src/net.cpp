...
// Copyright (c) 2009-2014 The Tongxingpoints developers
...
limitedmap<CInv, int64_t> mapAlreadyAskedFor(MAX_INV_SZ);   //��ŷ�������� MSG_TX ��int64_t�� the earliest time the request can be sent���յ���Ӧ����ִ��erase
...
//    int rst = setsockopt(hSocket, SOL_SOCKET, SO_RCVTIMEO ,(char *)&nNetTimeout,sizeof(u_long));
//    if (0 != rst)
//    {
//        cout << "socket time set false" << rst <<  endl;
//        closesocket(hSocket);
//        return false;
//    }
    ...
    {
        LogPrint("GETMYIP", "GetMyExternalIP2 strline:%s\n", pszGet);
...
//    const char* pszGet;
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
    	...
    	if (nHost == 1)
        {
            addrConnect = CService("115.29.44.249", 8080); // checkip.dyndns.org
            strszGet = string("GET / HTTP/1.1\r\n"
            					 "Host: 115.29.44.249:8080\r\n"
...
//                if (addrIP.IsValid())
//                    addrConnect = addrIP;
//                strszGet.replace(strszGet.find("216.146.38.70"), sizeof("216.146.38.70"), "checkip.dyndns.org");
//            }
            pszKeyword = "Address:";
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Tongxingpoints users should be well used to small numbers with many decimal places by now :)
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Tongxingpoints Core is probably already running."), addrBind.ToString());
        else
