...
{
cout << "begin SysTestBase::ImportAllPrivateKey" << endl;
	...
	for (int i = 0; i < nCount; i++) {
		...
		if (!CommandLineRPC_GetValue(sizeof(argv2) / sizeof(argv2[0]), argv2, value)) {
			cout << "get Value false" << endl;
			return false;
		}
	}
	cout << "end SysTestBase::ImportAllPrivateKey" << endl;
	return true;
...
//		int argc = 2;
//		char* argv[] = {"D:\\cppwork\\Tongxingpoints\\src\\Dacrsd.exe","-datadir=d:\\bitcoin" };
	assert(pThreadShutdown == NULL);
	{
	std::tuple<bool, boost::thread*> ret = RunTongxingpoints(argc, const_cast<char **>(argv));
